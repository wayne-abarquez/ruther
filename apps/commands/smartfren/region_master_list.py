...
class SmartfrenRegionMasterList (Command) :
    option_list = (
    ...
    def load(self, filename):
        with open(filename, 'r') as f:
            ...
            for obj in list_reader:
                ...
                boundary = Boundary.query.filter_by(extref_id = int(boundary_id), level_id = 2).all()
                
                if not len(boundary):
                    b = Boundary(extref_id = boundary_id, name = boundary_name, level_id = boundary_level_id, parent_id = 1)
                    db.session.add(b)
                    db.session.commit()
    ...
    def dump(self, filename):
        with open(filename, 'w') as f:
            ...
            for _b in Boundary.query.filter_by( level_id = 2 ):
...
manager.add_command('smartfren_region_master_list', SmartfrenRegionMasterList())
