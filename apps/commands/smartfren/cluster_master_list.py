...
class SmartfrenClusterMasterList (Command) :
    option_list = (
    ...
    def run(self, filename, logfile, action):
        ...
        if action:
            if action in SmartfrenClusterMasterList.valid_actions:
                _action = action
            else:
                print 'specified action is not valid'
                print 'valid actions:'
                for _ in SmartfrenClusterMasterList.valid_actions:
                    print _
                # end for
                sys.exit(0)
            # end if
    ...
    def load(self, filename):
        with open(filename, 'r') as f:
            ...
            for obj in list_reader:
                ...
                boundary = Boundary.query.filter_by(extref_id = int(boundary_id), level_id = 3).all()
                
                parent = Boundary.query.filter_by(extref_id = int(boundary_parent_id), level_id = 2).all()[0] if boundary_parent_id else None
                
                if not len(boundary):
                    b = Boundary(extref_id = boundary_id, name = boundary_name, level_id = boundary_level_id, parent_id = parent.id if parent else None)
                    db.session.add(b)
                    db.session.commit()
