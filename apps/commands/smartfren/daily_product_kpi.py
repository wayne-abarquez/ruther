...
class SmartfrenProductData (Command) :
    option_list = (
    ...
    def run(self, filename, action, date_str):
        ...
        if action:
            if action in SmartfrenProductData.valid_actions:
                _action = action
            else:
                print 'specified action is not valid'
                print 'valid actions:'
                for _ in SmartfrenProductData.valid_actions:
                    print _
                # end for
                sys.exit(0)
            # end if
    ...
    def aggregate (self, date_str):
        ...
        if date_str == 'all':
            ret = db.session.execute('SELECT DISTINCT ON (date_day, date_month, date_year) date_day, date_month, date_year FROM KPI')
            for obj in ret:
            ...
            ret = db.session.execute('SELECT DISTINCT ON (date_start_of_week) date_start_of_week FROM KPI')
            for obj in ret:
                start_week = obj[0]
            ...
            ret = db.session.execute('SELECT DISTINCT ON ( date_year) date_year FROM KPI')
            for obj in ret:
                year = obj[0]
    ...
    def aggregate_day (self, day, month, year):
        self.delete_tables ( SumFacilityProductGroupDay.__tablename__, year, month, day )     
        db.session.execute('SELECT aggregate_facility_productgroup_day_sum(:date_day, :date_month, :date_year)', { 'date_day' : day, 'date_month' : month, 'date_year' : year } )
        db.session.commit()
        
        self.delete_tables ( SumBoundaryProductDay.__tablename__, year, month, day )          
        db.session.execute('SELECT aggregate_boundary_product_day_sum(:date_day, :date_month, :date_year)', { 'date_day' : day, 'date_month' : month, 'date_year' : year } )
        db.session.commit()
        
        self.delete_tables ( SumBoundaryProductGroupDay.__tablename__, year, month, day )          
        db.session.execute('SELECT aggregate_boundary_productgroup_day_sum(:date_day, :date_month, :date_year)', { 'date_day' : day, 'date_month' : month, 'date_year' : year } )
        db.session.commit()
        
    
    def aggregate_week (self, week_date):
        day, month, year = week_date.day, week_date.month, week_date.year
        self.delete_tables ( SumFacilityProductWeek.__tablename__, start_week = week_date )   
        db.session.execute('SELECT aggregate_facility_product_week_sum(:date_day, :date_month, :date_year)', {'date_day' : day, 'date_month' : month, 'date_year' : year } )
        db.session.commit()
        
        self.delete_tables ( SumFacilityProductGroupWeek.__tablename__, start_week = week_date )         
