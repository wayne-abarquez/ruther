...
CREATE OR REPLACE FUNCTION aggregate_facility_product_month_sum ( int, int ) returns void as
$$
declare
...
p_year ALIAS FOR $2;
BEGIN
    v_tablename := 'sum_facility_product_month_y' || p_year::varchar;

IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
  
    EXECUTE 'CREATE TABLE ' ||  quote_ident(v_tablename) || 
    ' ( CONSTRAINT ' || quote_ident(v_tablename) || '_year CHECK ( date_year = ' || p_year::varchar || ' ))  INHERITS (sum_facility_product_month)';
...
END IF;
    ...
    EXECUTE format('INSERT INTO %I ( facility_id, facility_type_id, facility_boundary_id, facility_boundary_level_id, facility_boundary_parent_id, kpi_type, kpi, actual, target, data_point_count, product_id, date_month, date_year )
