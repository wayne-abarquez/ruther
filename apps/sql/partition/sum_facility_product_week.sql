...
CREATE OR REPLACE FUNCTION aggregate_facility_product_week_sum (int, int, int ) returns void as
$$
declare
...
v_end_year DATE;
BEGIN
    v_tablename := 'sum_facility_product_week_y' || v_year::varchar;
...
IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
    ...
    ' ( CONSTRAINT ' || quote_ident(v_tablename) || '_start_of_week CHECK ( date_start_of_week >= DATE ''' || to_char(v_start_year, 'YYYY-MM-DD') ||''' AND date_start_of_week < DATE '''|| to_char(v_end_year, 'YYYY-MM-DD') ||'''))  INHERITS (sum_facility_product_week)';
    
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_facility_index ON '|| quote_ident(v_tablename) || '( facility_id, product_id, kpi_type, date_start_of_week )';  
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_facility_type_index ON '|| quote_ident(v_tablename) || '( facility_type_id, product_id, kpi_type, date_start_of_week )';  

END IF;

    EXECUTE format('INSERT INTO %I ( facility_id, facility_type_id, facility_boundary_id, facility_boundary_level_id, facility_boundary_parent_id, kpi_type, kpi, actual, target, data_point_count, product_id, date_start_of_week, date_end_of_week )
        ...
        date_start_of_week, date_end_of_week, k.kpi_type, k.product_id
...
-- CREATE OR REPLACE FUNCTION sum_aggregation_facility_product_week_insert_trigger()
-- RETURNS TRIGGER AS $$
-- DECLARE
    -- v_year int;
    -- v_tablename varchar;
...
-- BEGIN
    ...
    -- EXECUTE 'CREATE TABLE ' ||  quote_ident(v_tablename) || 
     ...
     -- ' ))  INHERITS (sum_facility_product_week)';
...
-- CREATE TRIGGER sum_aggregation_facility_product_week_insert_trigger
    -- BEFORE INSERT ON sum_facility_product_week
    -- FOR EACH ROW EXECUTE PROCEDURE sum_aggregation_facility_product_week_insert_trigger();
