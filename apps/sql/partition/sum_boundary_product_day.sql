...
CREATE OR REPLACE FUNCTION aggregate_boundary_product_day_sum ( int, int, int ) returns void as
$$
declare
v_day ALIAS FOR $1;
v_month ALIAS FOR $2;
...
BEGIN
    v_tablename := 'sum_boundary_product_day_y' || v_year::varchar || '_m' || v_month::varchar;
...
IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
    ...
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_boundary_product_kpi ON '|| quote_ident(v_tablename) || '( boundary_id, product_id, facility_type_id, kpi_type, date_day, date_month, date_year )';  

END IF;

    EXECUTE format('INSERT INTO %I ( boundary_id, boundary_level_id, boundary_parent_id, facility_type_id, kpi_type, kpi, actual, target, data_point_count, product_id, calendar_id, raw_date, day_of_week, date_day, date_month, date_year, date_quarter, date_start_of_week, date_end_of_week )
        ...
        FROM kpi k WHERE date_day = $1 AND date_month = $2 AND date_year = $3
        GROUP BY  k.facility_boundary_id, k.facility_boundary_level_id, k.facility_boundary_parent_id, k.facility_type_id,
        k.raw_date, k.date_day, k.date_month, k.date_year, k.date_quarter, k.date_start_of_week, k.date_end_of_week, k.kpi_type, k.product_id, k.calendar_id, k.day_of_week
    ...
    LOOP
        IF ( v_boundary_level_desc.id != 1 ) THEN
            ...
            EXECUTE format('INSERT INTO %I ( boundary_id, boundary_level_id, boundary_parent_id, facility_type_id, kpi_type, kpi, actual, target, data_point_count, product_id, calendar_id, raw_date, day_of_week, date_day, date_month, date_year, date_quarter, date_start_of_week, date_end_of_week ) 
                ...
                k.calendar_id, k.raw_date, k.day_of_week, k.date_day, k.date_month, k.date_year, k.date_quarter, k.date_start_of_week, k.date_end_of_week
                FROM sum_boundary_product_day k 
