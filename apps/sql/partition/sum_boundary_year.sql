...
CREATE OR REPLACE FUNCTION aggregate_boundary_year_sum ( int ) returns void as
$$
declare
p_year ALIAS FOR $1;
v_tablename varchar;
...
BEGIN
    v_tablename := 'sum_boundary_year_y' || p_year::varchar;

IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
  
    EXECUTE 'CREATE TABLE ' ||  quote_ident(v_tablename) || 
    ' ( CONSTRAINT ' || quote_ident(v_tablename) || '_year CHECK ( date_year = ' || p_year::varchar || ' ))  INHERITS (sum_boundary_year)';
    
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_year_index ON '|| quote_ident(v_tablename) || '( boundary_id, facility_type_id, kpi_type, date_year )';  
...
END IF;
    ...
    EXECUTE format('INSERT INTO %I ( boundary_id, boundary_level_id, boundary_parent_id, facility_type_id, kpi_type, kpi, actual, data_point_count, date_year )
