...
CREATE OR REPLACE FUNCTION aggregate_boundary_product_month_sum ( int, int ) returns void as
$$
declare
p_month ALIAS FOR $1;
p_year ALIAS FOR $2;
...
BEGIN
    v_tablename := 'sum_boundary_product_month_y' || p_year::varchar;

IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
  
    EXECUTE 'CREATE TABLE ' ||  quote_ident(v_tablename) || 
    ' ( CONSTRAINT ' || quote_ident(v_tablename) || '_year CHECK ( date_year = ' || p_year::varchar || ' ))  INHERITS (sum_boundary_product_month)';
    
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_boundary_index ON '|| quote_ident(v_tablename) || '( boundary_id, product_id, facility_type_id, kpi_type, date_month, date_year )';  

END IF;

    EXECUTE format('INSERT INTO %I ( boundary_id, boundary_level_id, boundary_parent_id, facility_type_id, kpi_type, kpi, actual, target, data_point_count, product_id, date_month, date_year )
