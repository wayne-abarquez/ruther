...
CREATE OR REPLACE FUNCTION aggregate_facility_product_year_sum ( int ) returns void as
$$
declare
v_tablename varchar;
v_year ALIAS FOR $1;
...
IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
    ...
    ' ( CONSTRAINT ' || quote_ident(v_tablename) || '_year CHECK ( date_year = ' || v_year::varchar || ' ))  INHERITS (sum_facility_product_year)';
    
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_facility_index ON '|| quote_ident(v_tablename) || '( facility_id, product_id, kpi_type, date_year )';  
    EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_facility_type_index ON '|| quote_ident(v_tablename) || '( facility_type_id, product_id, kpi_type, date_year )';  

END IF;

    EXECUTE format('INSERT INTO %I ( facility_id, facility_type_id, facility_boundary_id, facility_boundary_level_id, facility_boundary_parent_id, kpi_type, kpi, actual, target, data_point_count, product_id, date_year )
