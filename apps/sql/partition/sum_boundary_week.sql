...
CREATE OR REPLACE FUNCTION aggregate_boundary_week_sum ( int, int, int ) returns void as
$$
declare
v_month ALIAS FOR $2;
v_year ALIAS FOR $3;
...
BEGIN
    v_tablename := 'sum_boundary_week_y' || v_year::varchar;
    ...
    IF ( EXISTS (SELECT relname FROM pg_class WHERE relname=v_tablename) = FALSE ) THEN
        ...
        '''))  INHERITS (sum_boundary_week)';
        
        EXECUTE 'CREATE INDEX ' || quote_ident(v_tablename) || '_boundary_index ON '|| quote_ident(v_tablename) || '( boundary_id, facility_type_id, kpi_type, date_start_of_week )';  

    END IF;

    EXECUTE format('INSERT INTO %I ( boundary_id, boundary_level_id, boundary_parent_id, facility_type_id, kpi_type, kpi, actual, data_point_count, date_start_of_week, date_end_of_week )
        ...
        k.date_start_of_week, k.date_end_of_week, k.kpi_type
